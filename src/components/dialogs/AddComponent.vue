...
const outputList = computed(() => {
	...
	if (filterSource.value.length > 0) {
		output = output.filter(
			(item) =>
				filterSource.value.findIndex((el) => item.source.includes(el)) > -1,
		)
	}
			...
			<div class="addcomponent-header">
				...
				<div class="addcomponent-header-search">
					...
					<div>
						...
						<button v-if="componentsSelected.length > 0" @click="handleSubmit">
					...
					<div v-for="item in outputList" :key="item.id">
						...
						<label :for="item.name" class="addcomponent-list-item">
							...
							<div>
								...
								<div>
									<ComponentTag
										v-if="item.chart_config.map_filter && item.map_config"
										icon="tune"
										text="篩選地圖"
									/>
									<ComponentTag
