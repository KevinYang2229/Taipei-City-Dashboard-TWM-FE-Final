...
const loadSvgContent = async () => {
	...
	try {
		const iconPaths = Array.isArray(props.chart_config.icon)
			? props.chart_config.icon.map(
				(icon) => `${BASE_URL}/images/map/${icon}.svg`
			)
			: [`${BASE_URL}/images/map/${props.chart_config.icon}.svg`]

		const responses = await Promise.all(
			iconPaths.map((iconPath) => fetch(iconPath))
		)

		const svgContentsArray = await Promise.all(
		...
		svgContents.value = svgContentsArray.filter((content) => content !== null)
	} catch (error) {
...
<template>
	<div v-if="activeChart === 'PairDonutChart'" class="donutchart">
		<apexchart
			width="100%"
			type="donut"
		...
		<div v-if="svgContents" class="donutchart-iconsbox">
			<div
				v-for="(svgContent, index) in svgContents"
				:key="index"
				class="donutchart-icon"
				v-html="svgContent"
				:style="`fill:${chart_config.color[index]}`"
			></div>
		</div>
		<div v-else class="donutchart-title">
			<h5>總合</h5>
			<h6>{{ sum }}</h6>
		</div>
	</div>
